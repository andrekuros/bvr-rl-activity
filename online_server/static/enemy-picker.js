...
const EnemyPicker = (() => {
  ...
  function drawWeights() {
    const box = document.getElementById("enemy-weights");
    if (!box || !catalog) return;
    const names = [...selected];
    if (!names.length) {
      box.innerHTML = "<p class='hint'>No opponents selected.</p>";
      return;
    }
    box.innerHTML = names
      .map((name) => {
        const w = weights[name] ?? 1;
        return `<div class="ew-row" data-name="${name}">
          <span class="ew-name">${name}</span>
          <input type="range" class="ew-slider" min="0" max="1" step="0.05" value="${w}" data-name="${name}" />
          <span class="ew-val" data-name="${name}">${Number(w).toFixed(2)}</span>
        </div>`;
      })
      .join("");
    box.querySelectorAll(".ew-slider").forEach((sl) => {
      sl.oninput = (e) => {
        const n = e.target.dataset.name;
        const v = Number(e.target.value);
        weights[n] = v;
        const lbl = box.querySelector(`.ew-val[data-name="${n}"]`);
  ...
  function toggle(name) {
    ...
    else { selected.add(name); if (weights[name] == null) weights[name] = 1; }
    draw();
    drawWeights();
  ...
  function selectAll() {
    (catalog.names || []).forEach((n) => { selected.add(n); if (weights[n] == null) weights[n] = 1; });
    draw();
    drawWeights();
    if (onChange) onChange(getSelected());
  }
  ...
  function render(containerId, cat, opts) {
    ...
    weights = {};
    [...selected].forEach((n) => (weights[n] = 1));
    if (opts.weights) {
      for (const [n, v] of Object.entries(opts.weights)) weights[n] = Number(v);
    }
  ...
  function getWeights() {
    const out = {};
    [...selected].forEach((n) => (out[n] = weights[n] ?? 1));
    return out;
  }

  function setSelected(names) {
    selected = new Set(names || []);
    [...selected].forEach((n) => { if (weights[n] == null) weights[n] = 1; });
    draw();
    drawWeights();
  }
